...
// $error = urlencode("Action impossible a définir !"); 
    ...
    if ( $action == "0" && isset( $_GET["Sid"] ) ) { // Test de chargement de l'id categorie en création de message avec ou sans sujet
        ...
        $html_formpost .= "<h4 style='color:lightseagreen;' align=center>Donnez un titre et saisissez votre message</h4>";
        ...
        $html_formpost .= '</br><input class="btn btn-lg btn-info" style="display:block; margin:auto;" type="submit" value="Envoyer le message">'; // Bouton envoyer en creation de message avec ou sans sujet
    } else {
    ...
    if ( $action == "1" && isset( $_GET["Cid"] ) && isset( $_GET["Slabel"] ) ) { // Test de chargement du labellé de sujet en création de sujet avec message
        ...
        $html_formpost .= "<h4 style='color:lightseagreen;' align=center>Saisissez le premier message de ce sujet</h4>";
    ...
    if ( $action == "2" && isset( $_GET["Uid"] ) && isset( $_GET["Sid"] ) && isset( $_GET["Pid"] ) ) { // Test de chargement de l'id post en modif et suppression
        ...
        $html_formpost .= "<h4 style='color:lightseagreen;' align=center>Saisissez les modifications du message</h4>";
        ...
        $html_formpost .= '</br><input class="btn btn-lg btn-info" style="display:block; margin:auto;" type="submit" value="Confirmez les modifications du message">'; // bouton confirmation en cas de modification
    } else {
    ...
    if ( $action == "3" && isset( $_GET["Uid"] ) && isset( $_GET["Sid"] ) && isset( $_GET["Pid"] ) ) { // Test de l'action en suppression et présence du nombre de posts par sujet
        ...
        $html_formpost = "<h4 style='color:lightseagreen;' align=center>Confirmez la suppression de ce message</h4>";
        ...
        include "postformodsup.php"; // Saisie des champs Titre et Texte avec affichage préalable des values
        $html_formpost .= '</br><input class="btn btn-lg btn-info" style="display:block; margin:auto;" value="Confirmez la suppression de ce message">'; // bouton confimation de suppression
    } else {
